...
public static class FunctionExportHelper
{
    public static IEnumerable<FunctionExport> LoadFunctionExports(string path)
    {
        ...
        foreach (var libraryPath in libraries)
        {
            ...
            var libraryVersion = libraryNameParts.Length > 1 ? int.Parse(libraryNameParts[1]) : 0;

            var exports = GetExports(libraryPath);
            foreach (var export in exports) yield return new FunctionExport { LibraryName = libraryName, LibraryVersion = libraryVersion, Name = export, ExportName = export  };
        }
    }
